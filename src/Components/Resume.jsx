...
function Resume() {
  ...
  function getSkillBars(data) {
		return data.map((skill) => {
			var className = "bar-expand " + (skillBarsInView ? `bar-${skill.level.replace(/\D/g, "")}` : "");
			return (
				<li key={skill.name}>
  ...
  <section id="resume">
    <div ref={educationRef} className="row education">
      <div className={`three columns header-col ${educationInView ? "slide-in-left" : "pre-animation"}`}>
        <h1>
          <span>Education</span>
        </h1>
    ...
    <div ref={workRef} className="row work">
      <div className={`three columns header-col ${workInView ? "slide-in-left" : "pre-animation"}`}>
        <h1>
          <span>Work</span>
        </h1>
      </div>
